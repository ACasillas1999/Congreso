...
if (empty($rows)) {
    if ($SILENT) {
        ...
        echo json_encode(["ok" => false, "error" => "Sin horario para este participante."]);
        exit;
    }
    http_response_code(404);
    exit("Sin horario para este participante.");
}
...
// ======== NUEVO: parser seguro para rangos de horario ========
function _fmt24($h, $m, $tag){
    $h = (int)$h; $m = (int)$m;
    $t = strtolower((string)$tag);
    if ($t !== '') {
        if (strpos($t,'p') !== false && $h < 12) $h += 12; // pm
        if (strpos($t,'a') !== false && $h == 12) $h = 0;  // 12am -> 00
    }
    return sprintf('%02d:%02d', $h, $m);
}

/**
 * Acepta: "9:30-10:30", "09:30 – 10:30", "9.30 a 10.30",
 *         "9:30am-10:30 am", "9:30 HRS" (sin fin)
 * Devuelve: ['ini'=>'HH:MM','fin'=>'HH:MM'|null]
 */
function parse_horario_rango($str){
    $s = trim((string)$str);
    $s = preg_replace('/[–—]/u','-',$s);      // guiones raros -> -
    $s = preg_replace('/\ba\b/iu','-',$s);    // " a " -> -
    preg_match_all('/(\d{1,2})[:\.](\d{2})\s*(a\.?m\.?|p\.?m\.?|am|pm)?/iu', $s, $m, PREG_SET_ORDER);
    if (!$m) return ['ini'=>null,'fin'=>null];
    $ini = _fmt24($m[0][1], $m[0][2], $m[0][3] ?? '');
    $fin = isset($m[1]) ? _fmt24($m[1][1], $m[1][2], $m[1][3] ?? '') : null;
...
if ($FORMAT === 'png') {
    ...
    foreach ($try_orients as $ORIENT) {
        ...
        if ($ORIENT==='portrait') {
            $M = ['l'=> $sx(60), 't'=> $sx(480), 'r'=> $sx(60), 'b'=> $sx(40)];
        } else {
            $M = ['l'=> $sx(80), 't'=> $sx(380),  'r'=> $sx(80), 'b'=> $sx(60)];
        }
        ...
        foreach ($days as $idx=>$d) {
            ...
            foreach ($byDay[$d] as $r) {
                ...
                $range   = parse_horario_rango($r['Horario']);
...
$primera = $horaTxt . " • " . $r['Salon'];
        ...
        // ======= NUEVO: Guardar archivo en disco y registrar en BD =======
        // 1) Asegurar carpeta
        if (!is_dir($OUTPUT_DIR)) {
            @mkdir($OUTPUT_DIR, 0777, true);
        }

        // 2) Nombre de archivo (evita colisiones)
        $orientTag = ($ORIENT === 'land') ? 'land' : 'port';
        $filename  = "Horario_{$ID_PARTICIPANTE}_{$orientTag}_" . date('Ymd_His') . ".png";
        $filePath  = rtrim($OUTPUT_DIR, "/") . "/" . $filename;

        // 3) Guardar PNG a disco
        imagepng($out, $filePath, 9);
        imagedestroy($out);

        // 4) Construir ruta pública (la que guardarás en BD)
        $publicPath = rtrim($PUBLIC_BASE, "/") . "/" . $filename;

        // 5) Guardar en la BD (columna Ruta_Horario)
        $upd = $conn->prepare("UPDATE participante SET Ruta_Horario = ? WHERE ID = ?");
        $upd->bind_param("si",$publicPath, $ID_PARTICIPANTE);
        $upd->execute();
        $upd->close();

        // 6) Devolver la imagen al navegador
       // 6) Responder según modo SILENT
if ($SILENT) {
    if (ob_get_length()) ob_end_clean();
    header("Content-Type: application/json");
    echo json_encode(["ok" => true, "ruta" => $publicPath, "file" => $filename]);
    exit;
} else {
    if (ob_get_length()) ob_end_clean();
